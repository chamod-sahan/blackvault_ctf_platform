...
export default function WindowsMachinesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const windowsMachines = allChallenges.filter((c: any) => c.os === 'WINDOWS');
        setMachines(windowsMachines);
      } catch (error) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="relative group">
        <div className="absolute -inset-1 bg-gradient-to-r from-blue-500/20 to-transparent rounded-xl blur-lg opacity-50" />
        <div className="relative bg-htb-card border border-htb-border rounded-xl p-8 overflow-hidden">
          ...
          <div className="flex items-center gap-3 text-blue-500 font-mono text-sm mb-4">
            <Terminal className="w-4 h-4" />
            <span className="tracking-[0.2em] uppercase">Target / Windows</span>
          </div>
          <h1 className="text-4xl font-black tracking-tighter mb-4 italic uppercase text-blue-500">Windows_Machines</h1>
          <p className="text-muted-foreground max-w-2xl font-medium leading-relaxed">
            Infiltrate enterprise Windows environments. Exploit Active Directory, bypass Kerberos, and escalate privileges to Domain Admin.
          </p>
        </div>
      </div>
      ...
      <div className="bg-htb-card/50 border border-htb-border p-4 rounded-xl">
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="SEARCH_WINDOWS_TARGETS..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="htb-input pl-10 h-11 border-transparent bg-background/50 focus:bg-background focus:ring-blue-500/50"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-blue-500 font-mono text-xs tracking-widest animate-pulse">SCANNING_NETWORK...</p>
        </div>
          ...
          {filteredMachines.map((machine) => {
            ...
            return (
              <Link
                ...
                className="htb-card group relative p-0 overflow-hidden flex flex-col transition-all duration-300 hover:ring-2 hover:ring-blue-500/20"
              >
                <div className="p-6 pb-4 flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-blue-500/10 rounded-xl flex items-center justify-center border border-htb-border group-hover:border-blue-500/30 transition-colors">
                      <Monitor className="w-6 h-6 text-blue-500" />
                    </div>
                    <div>
                      <h3 className="font-bold text-lg leading-tight group-hover:text-blue-500 transition-colors uppercase truncate max-w-[180px]">
                        {machine.title}
                      </h3>
                ...
                <div className="p-6 pt-4 mt-auto flex items-center justify-between border-t border-htb-border">
                  ...
                  <div className="htb-button-secondary py-1.5 px-4 h-auto text-[10px] border-blue-500/20 group-hover:bg-blue-500 group-hover:text-white transition-all uppercase">
                    Launch_Exploit
                  </div>
                </div>
              </Link>
        ...
        <div className="text-center py-32 htb-card border-dashed bg-transparent border-blue-500/20">
          <Monitor className="w-16 h-16 text-blue-500/20 mx-auto mb-6" />
          <h2 className="text-xl font-bold mb-2 uppercase tracking-widest text-muted-foreground">No Windows Targets</h2>
