...
export default function WriteupsPage() {
  ...
  const [selectedWriteup, setSelectedWriteup] = useState<{url: string, name: string} | null>(null);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20 px-2 lg:px-4">
      ...
      <div className="relative group">
        ...
        <div className="relative bg-htb-card border border-primary/30 rounded-xl p-8 sm:p-10 overflow-hidden shadow-2xl">
          ...
          <div className="flex items-center gap-3 text-primary font-mono text-sm mb-4">
            ...
            <span className="tracking-[0.2em] uppercase">Intel Reports</span>
          </div>
          <h1 className="text-4xl font-black tracking-tighter mb-4 italic uppercase text-primary">Mission_Writeups</h1>
          <p className="text-xl text-muted-foreground max-w-2xl font-medium leading-relaxed">
            Classified after-action reports and exploitation walkthroughs. Access restricted intel generated by top operators for vulnerable nodes.
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#0d0e12] border border-htb-border p-4 rounded-xl flex flex-col lg:flex-row gap-4">
        <div className="relative flex-1 group">
          ...
          <input
            type="text"
            placeholder="SCAN_REPORTS_BY_TARGET..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="htb-input pl-12 h-12 bg-background/50 border-htb-border focus:border-primary/50"
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 lg:w-1/2">
          <div className="relative flex-1">
            <select
              ...
              className="htb-input h-12 appearance-none cursor-pointer border-htb-border bg-background/50 pr-10 text-xs font-bold focus:border-primary/50"
            >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 gap-4">
          ...
          <p className="text-primary font-mono text-xs tracking-widest animate-pulse">LOCATING_INTEL_REPORTS...</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5 gap-4 lg:gap-5">
          {filteredChallenges.map((challenge) => {
            ...
            return (
              <button
                key={challenge.id}
                onClick={() => setSelectedWriteup({ url: challenge.writeupUrl!, name: challenge.title })}
                className={`text-left htb-card group relative p-0 overflow-hidden flex flex-col transition-all duration-300 hover:ring-1 hover:ring-primary/40 hover:translate-y-[-2px] shadow-xl ${challenge.solved ? 'opacity-90' : ''}`}
              >
                {/* Header Visual Area */}
                <div className="h-32 relative flex items-center justify-center overflow-hidden border-b border-htb-border bg-primary/5">
                   ...
                   <div className={`w-16 h-16 rounded-full flex items-center justify-center border-2 border-primary/30 group-hover:scale-110 transition-transform duration-500 shadow-2xl relative z-10 bg-[#0d0e12]`}>
                ...
                <div className="px-5 py-4 bg-muted/10 border-t border-htb-border flex items-center justify-between">
                   ...
                   <div className="text-[9px] font-bold text-primary group-hover:text-primary/80 hover:underline uppercase tracking-[0.2em] flex items-center gap-1 group-hover:gap-2 transition-all">
                      READ_INTEL <ChevronRight className="w-3 h-3" />
                   </div>
                </div>
              </button>
            );
        ...
        <div className="text-center py-32 htb-card border-dashed bg-transparent opacity-50">
          ...
          <h2 className="text-xl font-bold mb-2 uppercase tracking-widest text-muted-foreground">No_Reports_Detected</h2>
          <p className="text-[10px] font-mono uppercase">Writeup registry is empty for this classification.</p>
        </div>
      )}
      ...
      {selectedWriteup && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-50 flex flex-col animate-in fade-in duration-300" onContextMenu={(e) => e.preventDefault()}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground font-mono truncate max-w-sm">
                  {selectedWriteup.name} // CLASSIFIED
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-[9px] font-mono text-primary/40 uppercase tracking-widest border border-primary/20 px-2 py-1 rounded bg-primary/5 hidden sm:block">
          ...
          <div className="flex-1 relative bg-black/50 overflow-hidden group flex flex-col">
            {/* Overlay to catch and prevent right clicks and dragging across the iframe */}
            <div className="absolute inset-0 z-10 pointer-events-none group-hover:bg-primary/5 transition-colors duration-1000 mix-blend-overlay" />
            
            {selectedWriteup.url.toLowerCase().endsWith('.pdf') ? (
              <iframe
                src={`${getAssetUrl(selectedWriteup.url)}#toolbar=0&navpanes=0`}
                className="w-full h-full border-none z-0"
                title="Writeup Viewer"
              />
            ) : (
              <div className="w-full h-full overflow-y-auto p-4 sm:p-8 flex justify-center items-start z-0 relative">
                <img 
                  src={getAssetUrl(selectedWriteup.url)} 
                  alt="Writeup" 
                  className="max-w-4xl w-full h-auto select-none rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-htb-border" 
                  draggable={false} 
